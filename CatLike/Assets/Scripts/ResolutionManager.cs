...
public class ResolutionManager : MonoBehaviour
{
    ...
    private const string _name = "ResolutionManager";
    ...
    public static ResolutionManager Instance
    {
        get
        {
            if (_instance == null)
            {
                ...
                _instance = gameObject.AddComponent<ResolutionManager>();
    ...
    // 长宽比的宽度和高度
    [SerializeField]
    private float aspectRatioWidth = 1024;
    [SerializeField]
    private float aspectRatioHeight = 1366;

    [SerializeField]
    private int offsetWidth = 90;
    ...
    static extern long GetWindowLong32(IntPtr hWnd, int nIndex);
    ...
    public void SetAspectRatio(float newAspectWidth, float newAspectHeight, bool apply)
    {
        ...
        int goHeight = Mathf.RoundToInt(maxWidthPixel / aspect);
        if (maxHeightPixel > goHeight)
        {
            maxHeightPixel = goHeight;
        }
        else if(maxHeightPixel < goHeight)
        {
            maxWidthPixel = Mathf.RoundToInt(maxHeightPixel * aspect);
        }

        goHeight = Mathf.RoundToInt(minWidthPixel / aspect);
        if (minHeightPixel > goHeight)
        {
            minHeightPixel = goHeight;
        }
        else
        {
            minWidthPixel = Mathf.RoundToInt(minHeightPixel * aspect);
        }
        ...
        if (apply)
        {
            if (Screen.width > Screen.currentResolution.width)
            {
                ...
                {
                    int heightDamp = Screen.currentResolution.height - Screen.height;
                    int widthDamp = Screen.currentResolution.width - Screen.width;

                    if (Mathf.RoundToInt(heightDamp * aspect) < widthDamp)
                    {
                        useHeight = true;
                        height = Mathf.RoundToInt(Screen.currentResolution.width / aspect);
                    }
                }

                if (!useHeight)
                {
                    width = Mathf.RoundToInt(Screen.currentResolution.height * aspect);
                }
            }
            else if (Screen.height > Screen.currentResolution.height)
            {
                height = Mathf.RoundToInt(Screen.currentResolution.width / aspect);
            }
            else
            {
                int tempHeight = Mathf.RoundToInt(Screen.width / aspect);
                if(Mathf.Abs(height - tempHeight) > 1)
                {
                    height = tempHeight;
                }

                if(aspectRatioWidth < Screen.currentResolution.width && aspectRatioHeight < Screen.currentResolution.height)
                {
                    width = Mathf.RoundToInt(aspectRatioWidth);
                    height = Mathf.RoundToInt(aspectRatioHeight);
                }
            }
    ...
    private IntPtr WndProc(IntPtr hWnd, uint msg, IntPtr wParam, IntPtr lParam)
    {
        ...
        else if (msg == WM_NCLBUTTONDBLCLK) // 双击最大化
        {
            return default;
        }
        else if (msg == WM_SYSCOMMAND && wParam.ToInt32() == SC_MAXMIZE)
        {
            return default;
        }
...
//    }
    ...
    IEnumerator DelayedQuit()
    {
        ...
        //PlayerPrefs.SetInt("Screenmanager Is Fullscreen mode", Screen.fullScreen ? 0 : 1);
